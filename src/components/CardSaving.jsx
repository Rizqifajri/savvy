...
const CardSaving = ({ savingData, onDelete }) => {
  ...
  const increaseProgress = async () => {
    if (collected < savingData?.total_saving) {
      const newCollected = Math.min(collected + Number(savingData?.nominal), savingData?.total_saving);
  ...
  const handleDeleteSaving = async () => {
    ...
    try {
      ...
      onDelete(savingData.id); // Notify the parent component
    } catch (error) {
    ...
    <Card className='bg-[#392467] sm:overflow-hidden sm:max-w-[600px] sm:h-56 w-fit p-3'>
      <CardContent className='flex flex-col sm:flex-row w-fit gap-8'>
        <div className='flex flex-col justify-center'>
          <h1 className='mx-2 text-[20px] font-bold'>{savingData?.saving_method}</h1>
          <h3 className='text-center font-bold'>{savingData?.category_name}</h3>
          <ProgressBar totalSaving={collected} progressPercentage={progressPercentage} />
        </div>
